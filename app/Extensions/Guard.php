...
final class Guard implements MiddlewareInterface
{
  ...
  public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
  {
    ...
    if (!empty($request->getHeader('X-CSRF-Name'))) {
      $name = $request->getHeader('X-CSRF-Name')[0];
    }

    if (!empty($request->getHeader('X-CSRF-Value'))) {
      $value = $request->getHeader('X-CSRF-Value')[0];
    }
    ...
    if (in_array($request->getMethod(), ['POST', 'PUT', 'DELETE', 'PATCH'])) {
      ...
      if (!empty($request->getHeader('X-CSRF-Token'))) {
        $isValid = $_ENV['CSRF_TOKEN'] === $request->getHeader('X-CSRF-Token')[0];
      }
